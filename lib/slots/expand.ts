...
// Europe/Minsk, but we keep this layer timezone-agnostic and accept the
// Europe/Minsk → UTC conversion done by callers (or by Postgres).
//
...
export type SlotOccurrence = {
  /** ISO date in YYYY-MM-DD format (local Europe/Minsk date). */
  local_date: string;
  /** HH:MM (24h, local). */
 ...
 * happens in the SQL layer using `(timestamp ‖ ' Europe/Minsk') AT TIME ZONE`.
 */
...
/**
 * Build a UTC Date from local Europe/Minsk wall-clock components.
...
export function localWarsawToUTC(date: string, time: string): Date {
  ...
  // and let the server do the real Europe/Minsk conversion. This function is
