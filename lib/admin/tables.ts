...
export const TABLES: readonly TableDef[] = [
  ...
  // ---------------------------------------------------------------------------
  {
    ...
    pk: "id",
    defaultSort: { column: "created_at", ascending: false },
    searchColumns: ["name", "slug", "city", "description"],
    filterColumns: ["join_policy"],
    description:
      "Player-run clubs. Membership is in `club_members`; the `join_policy` column controls how players can join (approval / open / closed). Logos live in the `club-logos` storage bucket.",
    columns: [
      { key: "id", label: "ID", type: "uuid", readonly: true, hideInList: true },
      { key: "owner_id", label: "Owner", type: "uuid", required: true },
      { key: "slug", label: "Slug", type: "text", required: true },
  ...
  },
  {
    ...
    group: "people",
    pk: "id",
    defaultSort: { column: "applied_at", ascending: false },
    searchColumns: ["message"],
    filterColumns: ["status", "role", "is_primary"],
    description:
      "Membership of profiles in clubs. status: pending/approved/rejected; role: member/admin (co-admin). At most one approved primary club per user (enforced by partial unique index).",
    columns: [
      { key: "id", label: "ID", type: "uuid", readonly: true, hideInList: true },
      { key: "club_id", label: "Club", type: "uuid", required: true },
      { key: "user_id", label: "User", type: "uuid", required: true },
      {
        key: "status",
        label: "Status",
        type: "select",
      ...
      { key: "message", label: "Message", type: "textarea", hideInList: true },
      { key: "applied_at", label: "Applied", type: "datetime", readonly: true },
      { key: "decided_at", label: "Decided", type: "datetime" },
      { key: "decided_by", label: "Decided by", type: "uuid", hideInList: true },
      ...META_COLUMNS.slice(1),
    ],
  },
  ...
  },
  {
    ...
    columns: [
      ...
      { key: "groups_count", label: "Groups", type: "number", hideInList: true },
      { key: "advance_per_group", label: "Adv/group", type: "number", hideInList: true },
      { key: "playoff_size", label: "Playoff size", type: "number", hideInList: true },
      { key: "third_place_match", label: "3rd place", type: "boolean", hideInList: true },
      ...META_COLUMNS.slice(1),
    ],
  },
  {
    ...
    searchColumns: ["name"],
    filterColumns: [],
    description: "Groups for hybrid (group + playoff) tournaments. A, B, C, … by position.",
    columns: [
      { key: "id", label: "ID", type: "uuid", readonly: true, hideInList: true },
  ...
  },
  {
    ...
    searchColumns: [],
    filterColumns: ["status", "withdrawn"],
    description: "Player ↔ tournament registrations (with approval status).",
    columns: [
      { key: "id", label: "ID", type: "uuid", readonly: true, hideInList: true },
      { key: "tournament_id", label: "Tournament", type: "uuid", required: true },
      { key: "player_id", label: "Player", type: "uuid", required: true },
      ...
      { key: "group_id", label: "Group", type: "uuid", hideInList: true },
  ...
  },
  {
    ...
    columns: [
      ...
      { key: "bracket_slot", label: "Slot", type: "number", hideInList: true },
      {
        ...
        label: "Stage",
        type: "select",
        options: ["group", "playoff", "third_place"],
        hideInList: true,
      },
      { key: "group_id", label: "Group", type: "uuid", hideInList: true },
      {
