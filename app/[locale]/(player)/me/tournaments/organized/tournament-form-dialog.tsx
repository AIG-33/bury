...
type Props = {
  ...
  /**
   * The tournament is in progress / finished: format, discipline, seeding
   * and match rules are locked server-side, so the matching controls are
   * disabled (the 3rd-place toggle stays editable — it has its own flow).
   */
  locked?: boolean;
...
export function TournamentFormDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink-900/40 px-4 py-[max(1rem,env(safe-area-inset-top),env(safe-area-inset-bottom))]">
      {/* Column layout: header and footer stay put, only the form body
          scrolls — the Save button is always reachable on small screens. */}
      <div className="shadow-pop flex max-h-[85dvh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl bg-white">
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="flex min-h-0 flex-1 flex-col">
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-6 py-4">
            {locked && (
              <p className="rounded-lg bg-ball-50 px-3 py-2 text-xs text-ball-900">
                {copy.hints.locked_fields}
              </p>
            )}
            <div>
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <select
                      ...
                      disabled={locked}
                      className="h-10 w-full rounded-[13px] border border-[rgba(20,60,30,0.12)] bg-[#FBFDF9] px-3 text-sm outline-none focus:border-grass-400 focus:ring-2 focus:ring-grass-200 disabled:cursor-not-allowed disabled:opacity-60"
                    >
          ...
          <div className="shrink-0 border-t border-ink-100 px-6 py-4 pb-[max(1rem,env(safe-area-inset-bottom))]">
            {error && (
