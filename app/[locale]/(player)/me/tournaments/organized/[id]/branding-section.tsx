...
export function TournamentBrandingSection({
  ...
  const t = useTranslations("tournamentsOrganized.branding");
  ...
  async function upload(file: File, kind: Kind): Promise<string | null> {
    ...
    try {
      ...
      const { error: upErr } = await supabase.storage
        .from("tournament-branding")
        .upload(path, file, { upsert: true, contentType: file.type });
      if (upErr) {
        setError(upErr.message);
        return null;
      }
      const { data: pub } = supabase.storage.from("tournament-branding").getPublicUrl(path);
  ...
  function save() {
    startSave(async () => {
      setStatus("idle");
      setError(null);
      const r = await updateTournamentBranding({ tournament_id: tournamentId, branding: b });
      if (r.ok) {
        setStatus("saved");
        router.refresh();
      } else {
    ...
    <section className={SECTION}>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
        ...
        <div className="space-y-6">
          {/* Logo */}
          <Field label={t("logo")} hint={t("logo_hint")} icon={<ImageIcon className="h-3.5 w-3.5" />}>
          ...
          {/* Banner */}
          <Field label={t("banner")} hint={t("banner_hint")} icon={<ImageIcon className="h-3.5 w-3.5" />}>
            {b.banner_url && (
              // eslint-disable-next-line @next/next/no-img-element
              <img
                src={b.banner_url}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="block">
              <span className="mb-1 block text-sm font-medium text-ink-800">{t("title_override")}</span>
              <input
                type="text"
          ...
          <div className="flex items-center gap-3">
            <button type="button" onClick={save} disabled={saving || uploading != null} className={BTN_PRIMARY}>
        ...
        <div>
          ...
          <BrandingPreview theme={preview} branding={b} emptyLabel={t("preview_empty")} />
        </div>
      </div>
    </section>
  );
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-ink-100 shadow-inner">
      <div
        ...
        style={{ ...theme.backgroundStyle, color: theme.textColor }}
      >
        {/* Banner with scrim */}
          ...
          <div className="relative flex items-center gap-2">
            {theme.logoUrl && (
              ...
              <img
                ...
                style={{ borderColor: theme.accentColor ?? "transparent", background: "rgba(255,255,255,0.9)" }}
              />
            )}
            <span
            ...
            >
              {branding.title_override || "Minsk Open"}
            </span>
          </div>
        </div>
        ...
        <ul className="mb-2 space-y-2">
          {sponsors.map((s, i) => (
            <li key={i} className="flex flex-wrap items-center gap-2">
              {s.logo_url && (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={s.logo_url} alt="" className="h-8 w-8 rounded border border-ink-100 object-contain" />
              )}
              <input
                type="text"
                value={s.name}
