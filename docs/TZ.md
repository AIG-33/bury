...
### 5.5. Шаблоны турниров и панель организатора клуба

Ключевой сценарий для привлечения существующих лиг Минска: организатор настраивает турнир один раз и дальше запускает следующие этапы «в пару кликов».

- **Шаблон турнира** (`tournament_templates`): именованный слепок настроек (формат, правила матча, покрытие, площадки, взнос, видимость, посев, призы) без дат и участников. Payload — JSONB, валидируется Zod-схемой в `lib/tournaments/template-schema.ts`.
- Шаблон принадлежит создателю (`owner_id`) и опционально привязан к клубу (`club_id`) — тогда его видят и используют все со-админы клуба.
- «Сохранить как шаблон» — с карточки любого своего турнира. «Создать по шаблону» — выбрал шаблон, поставил дату старта → черновик турнира готов.
- **Панель организатора** в кабинете владельца клуба (`/me/clubs/owned/[id]`, секция «Турниры клуба»):
  - все турниры клуба (включая черновики) со статусами, числом участников и заявок;
  - кнопки «По шаблону», «Повторить последний», «Новый турнир»;
  - очередь матчей, ожидающих ввода счёта, по активным турнирам клуба;
  - управление шаблонами клуба (список, удаление).
- Управление конкретным турниром (сетка, счёт, заявки) остаётся за его организатором (`tournaments.owner_id`); панель клуба — сводный вид и точка быстрого создания.
